...
namespace CustomPropertyDrawering.SerializedDictionaries
{
    ...
    public abstract class SerializableDictionaryDrawer<K, V> : PropertyDrawer
    {
        ...
        protected T GetGenericTemplate<T>() where T : SerializableKeyValueTemplate<K, V> => ScriptableObject.CreateInstance<T>();
        ...
        private SerializedProperty GetTemplateKeyProp(SerializedProperty mainProp) => GetTemplateProp(templateKeyProp, mainProp);
        private SerializedProperty GetTemplateValueProp(SerializedProperty mainProp) => GetTemplateProp(templateValueProp, mainProp);

        private SerializedProperty GetTemplateProp(Dictionary<int, SerializedProperty> source, SerializedProperty mainProp)
        {
            if (!source.TryGetValue(mainProp.GetObjectCode(), out var p))
            {
        ...
        private readonly Dictionary<int, SerializedProperty> templateKeyProp = new Dictionary<int, SerializedProperty>();
        private readonly Dictionary<int, SerializedProperty> templateValueProp = new Dictionary<int, SerializedProperty>();

        private SerializedProperty GetKeysProp(SerializedProperty mainProp) => GetCachedProp(mainProp, "keys", keysProps);
        private SerializedProperty GetValuesProp(SerializedProperty mainProp) => GetCachedProp(mainProp, "values", valuesProps);
        ...
        private readonly Dictionary<int, SerializedProperty> keysProps = new Dictionary<int, SerializedProperty>();
        private readonly Dictionary<int, SerializedProperty> valuesProps = new Dictionary<int, SerializedProperty>();
        ...
        {
            if (!indexedPropertyDicts.TryGetValue(arrayProp.GetObjectCode(), out var d))
